...
fun Layout(
    ...
    when (structure) {
        ...
        is LayoutStructure.Single -> {
            val actions = remember(structure, onLayoutUpdate) {
                object: LayoutActions {
                    override fun close() {
                        onLayoutUpdate(LayoutStructure.Empty)
                    }

                    override val structure: LayoutStructure = structure
                }
            }
            CompositionLocalProvider(LocalLayoutActions provides actions) {
                structure.content()
            }
        }
